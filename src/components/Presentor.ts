...
export class Presenter {
    ...
    private setupEventListeners() {
        ...
        this.events.on(/^(order|contacts)\..*:change/, this.onFieldChange.bind(this));
    }

    private async loadProductList() {
        try {
            const response = await this.api.getProductList();
            this.appState.setProductList(response.items); // Используем response.items
        } catch (error) {
            console.error(error);
        }
    }
    ...
    private onBasketOpen() {
        const items = this.appState.basket.map(item => {
    ...
    private validateAndRender(component: any) {
        ...
        this.modal.render(component.render({
            valid: this.appState.isOrderValid,
            errors: component === this.order ? Object.values(this.appState.orderErrors) : Object.values(this.appState.contactErrors),
        }));
    }
    ...
    private onFormErrorsChange(errors: { orderErrors: IFormError, contactErrors: IFormError }) {
